...
- Add/remove/enable/disable/reorder addons, with explicit local-network permission,
  shared-addon reconfiguration and linking an installation to another profile.
- Per-profile playback preferences (subtitles by default, ordered audio/subtitle
  language priority, SDH/forced/audio-description/commentary), per-profile Trakt
  device login with sync and disconnect, and device-wide player display defaults
  (subtitle size, picture size, default speed).
...
This is the first TV client, not full Linux feature parity. PiP, transfer
management, subtitle-addon discovery, subtitle/audio timing offsets and TV
home-screen recommendations are not wired into the TV interface.
...
## Web settings and the browser remote

The TV runs a LAN-only settings server from `crates/madari-tv/src/web.rs`, bound
to `0.0.0.0:11471`. Open the address shown in **Settings → Web settings** and pair
with the eight-character code on that screen. The server rejects any request whose
`Host` is not a literal private IP on its own port, and any request whose `Origin`
is not that same origin.

### Stack

- **Server:** Axum + `utoipa`. The typed REST surface (`/api/pair`,
  `/api/overview`, `/api/profiles/*`, `/api/preferences`, `/api/addons/*`,
  `/api/remote`) is the only way in; there is no generic command dispatcher.
- **Spec and client:** `openapi.json` is generated by the `web_openapi` example
  and `@hey-api/openapi-ts` generates the SDK plus TanStack Query hooks from it.
  Regenerate with `pnpm run generate` inside `crates/madari-tv/web` after any
  change to the Rust DTOs. The generated client is committed, and CI fails if it
  is stale or if any file under `web/src` is missing from git.
- **Packages:** pnpm (`pnpm install`, lockfile committed). Settings that pnpm 10
  and later moved out of `package.json` live in `pnpm-workspace.yaml`; read the
  comment there before changing `minimumReleaseAge`.
- **UI:** Vite + React + TypeScript, Astryx components, StyleX for the little
  layout Astryx props cannot express. `web/dist` is embedded into the library
  with `include_str!`, so `scripts/build-tv-native.sh` builds the UI before
  cargo. Component styles compile from Astryx source via `astryxStylex()`; do
  not also import the prebuilt `astryx.css`.
- **Routing:** `createBrowserRouter`. Unknown paths render a real not-found
  screen; the server's SPA fallback serves `index.html` for non-`/api/`,
  extensionless GETs only.

### Sessions

Auth is a stateless HS256 JWT whose signing key lives in `web.key` beside
`profiles.sqlite`. Because the key survives a restart, **a paired browser stays
paired when the TV app restarts** — that is the whole point of the token, so do
not move the key into memory. `/api/session/refresh` re-issues a token before it
expires (30 days). `/api/logout` records the browser id in `web.revoked`, which
is also persisted, so a logged-out token cannot be replayed even after a restart.

Verified by `cargo test -p madari-tv --lib web`, which covers a forged token, an
expired token, a cleared in-memory cache (standing in for a restart), logout
revocation, and the remote command allowlist.

### Remote

`POST /api/remote {"command": "up"}` queues one key from a fixed allowlist. The
TV's poll loop reads and drains `commands` from `web_status` every 400 ms and
replays each key through `MainActivity.dispatchKeyEvent`, so the player, the
Compose focus system and click handling all see it exactly as if the physical
remote sent it. `back` goes through `onBackPressedDispatcher` and the volume keys
through `AudioManager`, because the framework handles those one level above the
view hierarchy.

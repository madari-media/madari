...
impl BindDevice {
    ...
    // Madari patch: iOS is an Apple platform with the same IP_BOUND_IF /
    // IPV6_BOUND_IF socket options. Upstream lists only macOS, so iOS fell into
    // the Linux arm below and failed to compile on `bind_device`.

...
impl OurFileExt for File {
    ...
    // 32-bit Unix off_t cannot represent positions beyond 2 GiB. FileExt uses
    // the platform's large-file API and retains the u64 position.
...
mod large_file_tests {
    use super::*;
    #[test]
    fn vectored_write_beyond_signed_32_bit_offset() {
        let path = std::env::temp_dir().join(format!("madari-large-offset-{}", std::process::id()));
        let file = std::fs::OpenOptions::new().read(true).write(true).create_new(true).open(&path).unwrap();
        let offset = i32::MAX as u64 + 4096;
        let result = file.pwrite_all_vectored(offset, [IoSlice::new(b"large"), IoSlice::new(b"offset")]);
        let mut bytes = [0; 11];
        if result.is_ok() { file.pread_exact(offset, &mut bytes).unwrap(); }
        drop(file);
        std::fs::remove_file(path).unwrap();
        assert_eq!(result.unwrap(), 11);
        assert_eq!(&bytes, b"largeoffset");
    }
}
